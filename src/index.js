...
const attack = function (target = null) {
  if (user.playerTurn() == true) {
    ...
    if (result.outcome == "Miss") {
      ...
      result.outcome == "Hit" ||
      result.outcome == "You sunk my battleship"
    ) {
      peg.classList.add("hit");
    }

    let tile = playerBoardDiv.querySelector(
      `[data-x='${result.x}'][data-y='${result.y}']`
    );
...
const onClick = function (e) {
  ...
  function getNearestTileElement(x, y) {
    try {
      if (
        boardBounds.left < x + target.offsetWidth &&
        x + target.offsetWidth < boardBounds.right &&
        boardBounds.top < y + target.offsetHeight &&
        y + target.offsetHeight < boardBounds.bottom
      ) {
        const closestTileX = Math.round(
          (x - boardBounds.left) / (tileWidth + 2)
        );
        const closestTileY = Math.round(
          (y - boardBounds.top) / (tileWidth + 2)
        );

        return boardContainer.querySelector(
          `[data-x="${closestTileX}"][data-y="${closestTileY}"]`
        );
      } else {
  ...
  function evaluateShipPosition() {
    if (
      boardBounds.right > currentX &&
      boardBounds.left < currentX &&
      boardBounds.top < currentY &&
      boardBounds.bottom > currentY
    ) {
      return true;
    } else return false;
  }
      ...
      if (element != null) {
        if (evaluateShipPosition()) {
          try {
            ...
            if (rotate) {
              direction =
                target.getAttribute("data-orientation") == "horizontal"
                  ? "vertical"
                  : "horizontal";
              move = playerBoard.moveShip(
                Number(target.getAttribute("data-size")),
                Number(tileStart.getAttribute("data-x")),
                Number(tileStart.getAttribute("data-y")),
                direction,
                Number(element.getAttribute("data-x")),
                Number(element.getAttribute("data-y")),
                target.getAttribute("data-orientation")
              );
            } else {
              move = playerBoard.moveShip(
                Number(target.getAttribute("data-size")),
            ...
            if (move) {
              target.style.left =
                element.getBoundingClientRect().x + window.scrollX + "px";
              target.style.top =
                element.getBoundingClientRect().y + +window.scrollY + "px";
            } else {
  ...
  function rotateOnClick() {
    ...
    const closestTile = getNearestTileElement(
